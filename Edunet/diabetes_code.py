...
def save_model(model, scaler, filename='diabetes_model_rf.sav'):
    # Save the trained model and scaler
    joblib.dump((model, scaler), filename)
    print(f"Model saved to {filename}")

def load_model(filename='diabetes_model_rf.sav'):
    # Load the saved model and scaler
    model, scaler = joblib.load(filename)
    return model, scaler

def predict_diabetes(model, scaler, custom_input, X_columns):
...
def main():
    ...
    X, y = load_and_preprocess_data("diabetes.csv")
    ...
    result = predict_diabetes(model, scaler, custom_input, X.columns)
